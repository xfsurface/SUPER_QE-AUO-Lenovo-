...
const ASSY_STATS = {
  get attendance() {
    return getAttendanceValueForDay("ASSY", 0);
  },
  get detection() {
    return getDetectionValueForDay("ASSY", 0);
  }
};
...
// Machine Status Table Component for ASSY (Consistent with BONDING & CUT)
function AssyStatusTable({ onRowClick }: { onRowClick: (row: any) => void }) {
  const appData = useAppData();
  const [filterLine, setFilterLine] = useState("ASSY-01#");
  const [isFilterOpen, setIsFilterOpen] = useState(false);

  const { allData, loading } = useStatusTableData("assy");
  ...
  const filteredData = allData.filter(d => d.line === filterLine);
...
// Yield & Productivity Trend Chart Component for ASSY
function AssyYieldProductionChart({ onOpenClosedLoop }: { onOpenClosedLoop?: () => void }) {
  const appData = useAppData();
  ...
  const { data: chartData, loading } = useTrendData("assy", days);
...
export function AssyDashboard({ onBack, onOpenClosedLoop }: { onBack: () => void, onOpenClosedLoop?: () => void }) {
  const appData = useAppData();
  ...
  const handleDownloadFile = (fileName: string) => {
    const downloadUrl = `/api/assy/files/download?name=${encodeURIComponent(fileName)}`;
    ...
    >
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <div className="relative">
            ...
            <h1 className="relative text-xl md:text-3xl font-black tracking-[0.4em] text-white uppercase drop-shadow-[0_0_15px_rgba(34,211,238,0.8)]">
              {appData?.labels?.dashboards?.assy?.title || "ASSY 智能管理看板"}
            </h1>
          </div>
      ...
      <div className="flex-1 p-6 flex flex-col gap-6 relative overflow-hidden bg-[#0a1526]">
        ...
        <div className="flex-1 flex gap-6 min-h-0">
          ...
          <div className="w-1/4 flex flex-col gap-6 z-10">
            <Card title={appData.labels.dashboards.cut.cards.attendance} className="flex-1">
              <div className="w-full flex justify-center items-center pt-2 gap-6">
                <Gauge 
                  value={ASSY_STATS.attendance.value} 
                  label={appData.labels.dashboards.cut.stats.attendanceRate} 
                  actual={ASSY_STATS.attendance.actual}
                  total={ASSY_STATS.attendance.total}
                  onClick={() => setModalTitle(appData?.labels?.dashboards?.assy?.stats?.attendance || "出勤率")} 
                ...
                />
                <Gauge 
                  value={ASSY_STATS.detection.value} 
                  label={appData.labels.dashboards.cut.stats.detectionRate} 
                  color="blue" 
                  actual={ASSY_STATS.detection.actual}
                  total={ASSY_STATS.detection.total}
                  unit="pcs"
                  onClick={() => setModalTitle(appData?.labels?.dashboards?.assy?.stats?.detection || "检出率")} 
                  gaugeClassName="w-48 h-36"
                  valueSize="text-base"
                  labelSize="text-base"
                  dataSize="text-xs"
            ...
            <Card title={appData.labels.dashboards.assy?.cards?.quality || "材料品质监控[Cell&POL材料不良]"} className="flex-1">
              <InlineTrendChart title={appData.labels.dashboards.cut.stats.materialYield} site="assy" />
            </Card>
          </div>
          ...
          <div className="flex-1 flex flex-col gap-6 z-10">
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                <div className="absolute left-2 bottom-2 text-[8px] text-cyan-500/30 font-mono">ASY_ACTIVE_STABLE</div>
              </div>
            ...
            <div className="flex-1 bg-[#102445]/60 border border-cyan-500/30 rounded-sm backdrop-blur-xl flex flex-col items-center justify-center relative group overflow-hidden shadow-[inset_0_0_60px_rgba(6,182,212,0.15)]">
              ...
              <div className="w-full h-full relative z-10 flex flex-col">
                <AssyYieldProductionChart onOpenClosedLoop={onOpenClosedLoop} />
              </div>
          ...
          {/* RIGHT COLUMN */}
          <div className="w-1/4 flex flex-col gap-5 z-40 relative">
            <AiAssistant workshop="ASY" />
            <Card title={appData.labels.dashboards.cut.cards.machine} className="min-h-0 overflow-hidden" style={{ flex: '0.9 0.9 0%' }}>
              <AssyStatusTable onRowClick={setSelectedParamRow} />
            </Card>
            <Card title={appData.labels.dashboards.cut.cards.standard} className="min-h-0 overflow-hidden relative" style={{ flex: '1.1 1.1 0%' }}>
              <div className="h-full flex flex-col justify-around py-1">
                {fileShelves.length === 0 ? (
                  ...
                  fileShelves.map((shelf, shelfIdx) => (
                    <div 
                      key={shelfIdx} 
                      className={`flex justify-around items-end px-2 pb-1.5 border-b border-cyan-500/20 relative ${shelfIdx > 0 ? "mt-4" : ""}`}
                    >
                      {shelf.map((doc, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-[14px] text-white/80 mt-1 whitespace-nowrap overflow-hidden text-ellipsis max-w-[100px] text-center font-medium" title={doc.name}>
                              {doc.name}
                            </span>
                          </motion.div>
                        );
              ...
              <AnimatePresence>
                {downloadToast && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 10 }}
                    className="absolute inset-x-0 bottom-0 bg-cyan-950/95 border-t border-cyan-500/30 py-2 px-3 flex items-center justify-between text-[9px] text-cyan-200 shadow-[0_-5px_15px_rgba(0,0,0,0.5)]"
                  >
        ...
        <div className="h-84 z-10 relative group mt-2">
          ...
          <div className="h-full bg-gradient-to-r from-[#102445]/90 via-[#0a1a2f]/80 to-[#102445]/90 border border-cyan-500/40 flex items-center justify-around text-cyan-100 shadow-[0_0_30px_rgba(6,182,212,0.1)] relative overflow-hidden px-4 gap-4">
            ...
            <div className="flex-1 h-full flex flex-col pt-6 pb-2">
              ...
              <div className="flex-1 bg-cyan-950/20 rounded-lg border border-cyan-500/10 overflow-hidden">
                <HumidityChart site="assy" />
              </div>
            </div>
            <div className="absolute left-2 bottom-1 text-[8px] text-white/40 font-mono">ID: ENV_SCAN_ASY_905</div>
          </div>
        </div>
      </div>
